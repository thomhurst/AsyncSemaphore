...
/// <summary>
/// Frozen snapshot of the core as of commit 96f3d58 (every grant handed straight to the head waiter,
/// before the overtakable grant of issue #589), kept so <see cref="AbBenchmarks"/> can A/B the
/// working-tree core against a fixed reference in the same run. Do not edit; regenerate from a newer
/// commit if a new baseline is wanted.
/// </summary>
public sealed class BaselineAsyncSemaphore
{
    ...
    /// </summary>
    private ValueTask<BaselineReleaser> EnqueueTimedWaiter(TimeSpan timeout, CancellationToken cancellationToken)
    {
        if (timeout == TimeSpan.Zero)
        {
