...
# resolves from -OwnerId, ASYNCSEMAPHORE_AGENT_LOCK_OWNER_ID, or CODEX_THREAD_ID (in that order).
...
#   $env:ASYNCSEMAPHORE_AGENT_LOCK_OWNER_ID = 'my-agent-run-id'
...
$stateRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'asyncsemaphore-agent-locks'
...
function Resolve-OwnerIdentity([string]$Explicit) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($env:ASYNCSEMAPHORE_AGENT_LOCK_OWNER_ID)) {
        return $env:ASYNCSEMAPHORE_AGENT_LOCK_OWNER_ID.Trim()
    }
...
if (-not $OwnerId) {
    Die 2 'cannot resolve lock owner identity: pass -OwnerId or set ASYNCSEMAPHORE_AGENT_LOCK_OWNER_ID (CODEX_THREAD_ID is used automatically in Codex)'
}
...
$lockKey = "asyncsemaphore:agent-lock:$LockName"
$metaKey = "asyncsemaphore:agent-lock-meta:$LockName"
